...
@php($fecha_principal = Jenssegers\Date\Date::parse($inversionBase->fecha_inicio)->format('l d F'))
@php($año_principal = Jenssegers\Date\Date::parse($inversionBase->fecha_inicio)->format('Y'))
...
<body>
  ...
  <strong> <p class="text-justify
                text-break mt-5">ADENDUM DE AMPLIACIÓN DE VIGENCIA AL CONTRATO DE MUTUO CON INTERESES DE FECHA 
                {{ Str::upper(\Jenssegers\Date\Date::parse($fecha_principal)->format('l d F')) }} DEL {{ $año_principal }}
                IDENTIFICADO 
                CON NÚMERO DE FOLIO {{ $inversionBase->folio }}, EN LO SUCESIVO EL CONTRATO; QUE CELEBRAN {{$empresa->nombre}}, REPRESENTADA EN ESTE 
            ...
            celebraron el Contrato de Mutuo con Intereses identificado con el número de folio {{ $inversionBase->folio }}, en lo sucesivo <strong>EL CONTRATO</strong>.
            </p>
            <p class="text-justify text-break"><strong>II.</strong> Que en fecha <u>{{ Str::upper(\Jenssegers\Date\Date::parse($fecha_inicio)->format('l d F')) }} DEL {{ $año }}</u> el <strong>CLIENTE</strong> solicitó la 
                <strong>AMPLIACIÓN DE LA VIGENCIA y EL INCREMENTO DE LA CANTIDAD</strong> entregada en mutuo a través de la plataforma <u>https://finverr.com.</u>
            </p>
            <h3 class="h3 text-center">DECLARACIONES</h3>
            <p class="text-justify text-break">  <strong>I.</strong> Declara el <strong>LAS PARTES:</strong></p>
            <p class="text-justify text-break"><strong>a)</strong> Que reconocen la existencia de <strong>EL CONTRATO</strong> y se reconocen 
            mutuamente la capacidad legal y personalidades con las que se ostentan para la celebración de presente Adendum.
            </p>
            <p class="text-justify text-break"><strong>b)</strong> Que a <strong>LAS PARTES</strong> se encuentran al corriente del
             cumplimento de obligaciones contraídas en <strong>EL CONTRATO.</strong>
            </p>
            <p class="text-justify text-break"><strong>c)</strong> Que <strong>LAS PARTES</strong>, de común acuerdo, han Convenido 
            actualizar los montos de la cantidad entregada en Mutuo y de la vigencia de <strong>EL CONTRATO.</strong>
            </p>
            ...
            <p class="text-justify text-break">  <strong>III.</strong> Declaran las <strong>PARTES</strong>:</p>
            ...
            correspondiente mediante correo electrónico a la dirección señalada por el <strong>CLIENTE.
            </p>
            <p class="text-justify text-break">Con lo anterior se tiene por cierto y manifestado que ha aceptado los términos y condiciones de 
                    ...
                    <p class="text-justify text-break">Este Contrato se suscribirá por <strong>LAS PARTES</strong> a través de medios electrónicos, 
                        para lo cual cada una de <strong>LAS PARTES</strong> aquí reconocen que los procesos informáticos utilizados,  
                         </p>
                         <p class="text-justify text-break">permiten expresar 
                ...
                <p class="text-justify text-break"><strong>2.- <U>OBJETO.</U> LAS PARTES</strong> convienen modificar las Cláusulas <strong>2.-, 3.-</strong>, en 
                caso de proceder, y <strong>4.-</strong> de <strong>EL CONTRATO</strong>, en lo relativo a la cantidad entregada en mutuo, el porcentaje de intereses y a la vigencia, respectivamente.
                </p>
                <p class="text-justify text-break"><strong>3.- <U>INCREMENTO.</U> LAS PARTES</strong> convienen el INCREMENTO de la cantidad de dinero entregada 
                en mutuo estipulada en las <strong>Cláusula 2.-</strong> de <strong>EL CONTRATO</strong> para quedar en  <u>{{ $inversion->cantidad }}
                ({{ $cantidadLetras }}).,</u> por lo que en este acto <strong>EL CLIENTE</strong> entrega a 
                <strong>LA EMPRESA</strong> la cantidad de <u>${{ $diferenciamayorFormateada  }}
                (${{ $cantidadLetrasmayor }}).,</u> para la actualización del <strong>INCREMENTO.</strong>
                </p>
            <p class="text-justify text-break">
            <strong>4.- <u>INTERESES ORDINARIOS.</u> LAS PARTES</strong> acuerdan que la actualización de la cantidad
             de dinero entregada en mutuo <u>{{ $mensajeInteresesmayor }}</U> estipulados en la Cláusula 3.- de <strong>EL CONTRATO.</strong>
            </p>

                <p class="text-justify text-break"><strong>5.- <u>VIGENCIA.</u> LAS PARTES</strong> convienen la AMPLIACIÓN de la vigencia 
                    estipulada en la Cláusula 2.- de <strong>EL CONTRATO,</strong> la cual, se actualiza al <u>{{ $fecha_fin }} del {{ $año_fin }}</u>.
                </p>
                <p class="text-justify text-break"><strong>6.- <u>OBLIGACIONES.</u></strong> El presente Adendum no representa una novación en las obligaciones en el Contrato 
                    Principal de Mutuo con Intereses, por lo que <strong>LAS PARTES</strong> siguen obligadas en los términos y las condiciones anteriormente establecidas en el Contrato Principal.</p>
                <p class="text-justify text-break"><strong>7.- <u>JURISDICCIÓN.</u> LAS PARTES</strong> acuerdan que toda controversia e interpretación que se derive de 
                    este Adendum, respecto de su operación, formalización y cumplimiento, será resuelta conforme lo estipulado en <strong>EL CONTRATO</strong>. </p>
                ...
                <p class="text-justify text-break">Adendum firmado electrónicamente por {{$empresa->nombre}}, a través de su representante legal 
                    en carácter de <strong>PRESTADOR</strong> y {{ $user->name . ' ' . $user->lastName }} en carácter de <strong>CLIENTE.</strong></p>
                    <p class="text-justify text-break">Firmado electrónicamente el: <u>{{$inversion->fecha_reinversion}}</u> con clave de Firma: <u>{{$inversion->clave_reinversion}}</u>
