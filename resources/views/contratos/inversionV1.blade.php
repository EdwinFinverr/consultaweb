...
@endif</strong>a partir de la firma electrónica, la cual se realiza a través de la página web <u>https://finverr.com</u> por medio del procedimiento indicado en la Cláusula 1.-. Al concluir este Contrato, el <strong>CLIENTE</strong> recibirá la devolución íntegra de su 
        ...
        <p class="text-justify text-break">
            ...
            @foreach ($proyecto as $proyectos)
        <p class="text-justify text-break">Contrato firmado electrónicamente por FINVERR {{$empresa->nombre}}, a través de su representante 
            legal en carácter de <strong>PRESTADOR</strong> y <u>{{ $user->name . ' ' . $user->lastName }}</u> en carácter de <strong>CLIENTE</strong>
        </p>
        @endforeach
        <p class="text-justify text-break">Firmado electrónicamente el: <u>{{$inversion->fecha_firma}}</u> con clave de Firma: <u>{{$inversion->clave_firma}}</u>
        </p>
    </div>
